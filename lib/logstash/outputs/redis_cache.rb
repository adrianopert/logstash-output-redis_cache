...
class LogStash::Outputs::Redis_Cache < LogStash::Outputs::Base
  ...
  # The name of a field in event that will become in Redis key.
...
#  end
	...
	# Esto es solo para batch, no?
	# El buffer asi nomas no sirve, porque "agrupa" los eventos.  A priori, cada evento es un string, con lo cual no podemos obtener la clave de aqui (si fueran hashes)
	# Si queremos una version que bufferice, tenemos que laburarlo un poco.
...
#    congestion_check(key) unless teardown
    ...
    #@redis.rpush(key, events)
		# Si llegamos a usar el modo bufferizado, tendriamos que mandar n "eventos" a la vez
    @redis.mset(key, events)
  end
  ...
  def teardown
    if @batch
      buffer_flush(:final => true)
		...
		# Adriano: ver que onda si no es ni channel ni list!
